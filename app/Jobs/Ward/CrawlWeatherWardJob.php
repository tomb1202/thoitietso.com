...
class CrawlWeatherWardJob implements ShouldQueue
{
    ...
    public function handle()
    {
        $ward = $this->ward;
        try {
            if (!$ward || !$ward->url) return false;

            $url = rtrim($ward->url, '/') . '/theo-gio';
            $maxRetries = 5;
            $retryCount = 0;
            ...
            while ($retryCount < $maxRetries) {
                ...
                try {
                    ...
                    $crawler->filter('.weather-day')->each(function ($node) use ($ward, &$currentDate, &$lastHour, $sunrise, $sunset, $getText, $getAttr, $toFloat) {
                    ...
                    Log::info('✅ Fetched hourly weather for ward', [
                        'ward_id' => $ward->id,
                        'url' => $url,
                        'proxy' => $proxy,
                    ]);
                    ...
                    if ($retryCount >= $maxRetries) {
                        Log::error('❌ Failed hourly weather for ward after retries', [
                            'ward_id' => $ward->id,
                            'proxy' => $proxy,
