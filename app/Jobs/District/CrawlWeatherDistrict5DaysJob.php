...
class CrawlWeatherDistrict5DaysJob implements ShouldQueue
{
    ...
    {
        $this->districtId = $district instanceof District ? $district->id : (int)$district;
    }
    ...
    public function handle(): void
    {
        ...
        if (!$district || !$district->url) {
            Log::warning('District not found or missing URL for 5-day crawl', ['district_id' => $this->districtId]);
            return;
        }

        $url        = rtrim($district->url, '/') . '/5-ngay-toi';
        $today      = Carbon::today();
        ...
        while ($retryCount < $maxRetries) {
            ...
            try {
                ...
                $crawler->filter('#accordionExample details.weather-day')->each(function ($dayNode) use (
                    $district, $minDayDate, $getText, $getAttr, $toFloat
                ) {
                    ...
                    $dayNode->filter('.weather-content .weather-summary')->each(function ($hNode) use (
                        $district, $dayDate, $sunrise, $sunset, $getText, $getAttr, $toFloat
                    ) {
                ...
                Log::info('Crawled 5-day forecast for district', [
                    'district_id' => $district->id,
                ...
                if ($retryCount >= $maxRetries) {
                    Log::error('Failed to crawl district forecast after max retries', [
                        'district_id' => $district->id,
                        'district'    => $district->name,
                        'url'         => $url,
                        'proxy'       => $proxy,
                ...
                Log::warning('Retrying district crawl', [
                    'district_id' => $district->id,
                    'district'    => $district->name,
                    'proxy'       => $proxy,
                    'retry'       => $retryCount,
